...
import { ArrowLeft, Plus, Copy, Trash2, GitBranch, Star, ChevronRight } from "lucide-react";
...
export default function ProjectDetail() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (v) => {
      ...
      setShowCreate(false);
      setForm({ name: "", description: "", is_baseline: false });
      setCloneFrom("");
              ...
              <div className="absolute top-4 right-4 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    setCloneFrom(v.id);
                    setForm({ name: `${v.name} (Kopie)`, description: "", is_baseline: false });
                    setShowCreate(true);
                  }}
